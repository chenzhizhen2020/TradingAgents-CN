...
class SimpleAnalysisService:
    ...
    async def list_all_tasks(
        ...
        try:
            task_status = None
            db_status_values: Optional[List[str]] = None
            if status:
                try:
                    status_mapping = {
                        "processing": "running",
                    ...
                    db_status_values = ["processing", "running"] if status == "processing" else [task_status.value]
                except ValueError:
    ...
    async def list_user_tasks(
        ...
        try:
            ...
            # 1) 从内存读取任务
            logger.info(f"📋 [Tasks] 准备从内存读取任务: user_id={user_id}, status={status} (mapped to {task_status}), batch_id={batch_id}, limit={limit}, offset={offset}")
            ...
            if batch_id:
                tasks_in_mem = [task for task in tasks_in_mem if task.get("batch_id") == batch_id]
            logger.info(f"📋 [Tasks] 内存返回数量: {len(tasks_in_mem)}")
            ...
            try:
                ...
                if db_status_values:
                    # MongoDB 使用 processing，内存任务使用 running，两者都要兼容。
                    query["status"] = {"$in": db_status_values} if len(db_status_values) > 1 else db_status_values[0]
                    logger.info(f"📋 [Tasks] 添加状态过滤: {db_status_values}")
    ...
    async def get_batch_summary(self, user_id: str, batch_id: str) -> Optional[Dict[str, Any]]:
        ...
        return await BatchSummaryService(db_getter=get_mongo_db).get_batch_summary(user_id, batch_id)
    ...
    async def _save_analysis_result_web_style(self, task_id: str, result: Dict[str, Any]):
        ...
        try:
            ...
            task_doc = await db.analysis_tasks.find_one({"task_id": task_id}, {"batch_id": 1})
            batch_id = result.get("batch_id") or (task_doc or {}).get("batch_id")
